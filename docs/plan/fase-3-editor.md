...
| 3.2 | **Yjs backend** | Persistencia de updates en `roomUpdate`, snapshots (`roomSnapshot`), API de draft | `09` §2, `13` §4 | Al reabrir el editor el doc se reconstruye exactamente |
...
| 3.9 | **Publicación** | Draft vs. versiones (`roomVersion`), empaquetado de assets a R2, `assetsHash`, `packageFormat` | `08` §5–6, `13` §4 | Publicar congela la versión; `validate` se repite server-side |
...
| 3.12 | **Admin: settings y pricing** | `GET/PATCH /api/admin/settings/:key` y `/api/admin/pricing-tiers` | `13` §10, `02` §3.2 | Se cambia `maxPlayersPerRoom` y una fila de tramo sin romper histórico |
