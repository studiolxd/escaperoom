...
| 0.2 | **Infra local + datos** | Docker Compose (base de SLXD podada: sin ClickHouse ni Gatus) con PostgreSQL, PgBouncer, Redis, MinIO (S3 local) y **añadido de LiveKit + coturn**; migraciones SQL con **Prisma** en `packages/shared/prisma`; seed con Prisma Client | `14`, `03` §5.2, ADR-015 | `pnpm db:migrate` aplica 0001–0010 desde cero; `pnpm db:seed` carga admin + creador + Rey Aldric publicado |
| 0.3 | **Auth + usuarios/orgs** | **Better Auth** con plugin de organización y **modelo canónico** (tablas camelCase `user`, `session`, `account`, `verification`, `organization`, `member`, `invitation`; migración `0003_auth`); email mágico + Google; sesión por cookie + Bearer; `/api/auth/*` y `/api/me` | `13` §2, `14` §3, ADR-016 | Registro/login OAuth y email; `/api/me` devuelve perfil con organizaciones y rol |
...
| 0.7 | **Analítica base** | Endpoint de colección + cola Redis + worker → `analyticsEvent`; emisión desde Next API | `16`, `14` §8 | Insertar un evento de prueba desde API llega a `analyticsEvent` sin bloquear la request |
| 0.8 | ~~**Import script SLXD**~~ **CANCELADO** | No se importan **datos** de SLXD (decisión 2026-09-22): solo se reutiliza su **código** (ADR-017, `reference/reutilizacion-slxd.md`). El subsistema de identidad/orgs/créditos se copia/adapta, no se migra | `15` §2, ADR-014/017 | — (cancelado) |
