 ...
 * producto (ticket 1.1): Phaser renderiza la habitación activa con primitivas
 * isométricas y este overlay React permite cambiar entre las 3 subrooms.
 *
 * Solo se pasan al cliente los datos del modelo (sin secretos de puzzles),
 * serializados por el Server Component de la ruta.
 */
export function RoomPreviewShell({ model }: { model: RuntimeModel }) {
  const t = useTranslations("RoomPreview");
  ...
  return (
    <section className="relative h-[calc(100dvh-2rem)] w-full overflow-hidden rounded-xl border border-white/10 bg-slate-950">
      <RoomPreviewCanvas model={model} roomId={activeRoomId} />
